...
Common issues and solutions when using moltis on Termux.
...
If the above steps don't help, fully close and reopen the Termux app, then run `moltis gateway`. Rebooting the phone will reliably clear all state.
...
The `NODE_OPTIONS` environment variable in `~/.bashrc` still references the old installation path (`.moltis-lite`). This happens when updating from an older version where the project was named "moltis Lite".
...
```bash
curl -sL https://raw.githubusercontent.com/AidanPark/moltis-android/main/update.sh | bash && source ~/.bashrc
```

Or manually fix it:

```bash
sed -i 's/\.moltis-lite/\.moltis-android/g' ~/.bashrc && source ~/.bashrc
```
...
After running `moltis update`, moltis tries to restart the gateway service using `systemctl`. Since Termux doesn't have systemd, the `systemctl` binary doesn't exist and the command fails with `ENOENT`.

### Impact

**This error is harmless.** The update itself has already completed successfully — only the automatic service restart failed. Your moltis installation is up to date.
...
## sharp build fails during `moltis update`
...
When `moltis update` runs npm to update the package, it spawns npm as a subprocess. The Termux-specific build environment variables required to compile `sharp`'s native module (`CXXFLAGS`, `GYP_DEFINES`, `CPATH`) are set in `~/.bashrc` but are not automatically available in that subprocess context.

### Impact

**This error is non-critical.** moltis itself has been updated successfully — only the `sharp` module (used for image processing) failed to rebuild. moltis works normally without it.
...
```bash
bash ~/.moltis-android/scripts/build-sharp.sh
```

Alternatively, use `update.sh` instead of `moltis update` — it sets the required environment variables and rebuilds sharp automatically:

```bash
curl -sL https://raw.githubusercontent.com/AidanPark/moltis-android/main/update.sh | bash
```
...
```bash
curl -sL https://raw.githubusercontent.com/AidanPark/moltis-android/main/bootstrap.sh | bash
```
